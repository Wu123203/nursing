  ...
  <div class="employee-page">
    ...
    <div class="search-box">
      <el-form :inline="true" :model="query" @submit.prevent>
        <el-form-item>
          <el-input v-model="query.username" placeholder="请输入工号" clearable @keydown.enter="handleSearch" class="search-input">
            <template #prefix>
    ...
    <div class="container">
      ...
      <el-table v-loading="loading" :data="tableData" class="custom-table">
        ...
        <el-table-column prop="entryTime" label="入职时间" min-width="120" align="center">
          <template #default="scope">
    ...
    <!-- 添加员工 -->
    <el-dialog v-model="dialogVisible" width="40%" class="employee-dialog">
      <template #header>
      ...
      <el-form :model="form" :rules="rules" ref="form" label-position="right" label-width="80px" class="employee-form">
        <el-row :gutter="15">
          <el-col :span="12">
            <el-form-item label="工号" prop="username">
              <el-input v-model="form.username" placeholder="请输入工号" autocomplete="off">
                <template #prefix>
                  <el-icon><Postcard /></el-icon>
                </template>
              </el-input>
...
export default {
  ...
  methods: {
    ...
    load() {
      this.loading = true
      this.$http.get("/employee", { params: this.query }).then(res => {
        this.loading = false
    ...
    handleOff(row) {
      ...
      this.$confirm('确定要冻结账户吗？', '提示', {
        ...
        this.$http.put("/employee", { "id": row.id, "status": 1 }).then(res => {
          console.log(res)
          if (res.code === 200) {
            this.$message({
              type: "success",
    ...
    handleOn(row) {
      ...
      this.$confirm('确定要启动账户吗？', '提示', {
        ...
        this.$http.put("/employee", { "id": row.id, "status": 0 }).then(res => {
          console.log(res)
          if (res.code === 200) {
            this.$message({
              type: "success",
